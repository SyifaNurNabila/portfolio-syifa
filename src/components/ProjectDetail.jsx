...
const ProjectDetails = () => {
  ...
  // }, [slug]);
    ...
    useEffect(() => {
      ...
      const selectedProject = storedProjects.find((p) =>
        toSlug(p.title || p.Title || "") === slug
      );

      if (!selectedProject) {
        console.error("Project tidak ditemukan:", slug);
        return;
      }

      if (!storedProjects.length) {
        console.warn("No projects in localStorage");
      }
  ...
  return (
    <>
      <Helmet>
        <title>{project.title} — Syifa Nur Nabila</title>
        <meta
          name="description"
          content={
            project.description
              ? project.description.slice(0, 155)
              : `Project ${project.title} Syifa Nur Nabila — Software & Data Engineer.`
          }
        />
        ...
        <meta
          property="og:title"
          content={`${project.title} — Syifa Nur Nabila`}
        />
        <meta
